...
import { consumerRegSchemaPageOneEn, consumerRegSchemaPageOneBn } from "@/validations";
import { TConsumerRegisterData } from "@/interfaces";
import { useCreateProviderAccountMutation } from "@/redux/features/auth.features";
import { modifyRegistrationInfo } from "@/utils/modifyRegistrationInfo";
import { showToast } from "@/components/Reusable/CustomToast";

const GeneralProviderRegForm = ({ setPages, pages, setEmail }: TSetPage) => {
  const [createProvider, { isLoading }] = useCreateProviderAccountMutation();
  ...
  const onSubmit: SubmitHandler<TConsumerRegisterData> = async (data) => {
    try {
      ...
      const result = await createProvider(info).unwrap();
...
export default GeneralProviderRegForm;
