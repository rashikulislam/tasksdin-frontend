...
import { useCreateAgentAccountMutation } from "@/redux/features/auth.features";
import { modifyRegistrationInfo } from "@/utils/modifyRegistrationInfo";
import { showToast } from "@/components/Reusable/CustomToast";

const AgentPartnerRegForm = ({ setPages, pages, setEmail }: TSetPage) => {
  const [createAgent, { isLoading }] = useCreateAgentAccountMutation();
  ...
  const onSubmit: SubmitHandler<TConsumerRegisterData> = async (data) => {
    try {
      ...
      const result = await createAgent(info).unwrap();
...
export default AgentPartnerRegForm;
